...
import {
  ...
  parseArgs,
  requireEnv,
} from "./remove-backblast-attendance";
...
describe("helpers", () => {
  it("finds a beatdown when present", async () => {
    ...
    expect(beatdown?.ao_id).toBe("C123");
  });
  ...
  it("deletes attendance rows", async () => {
    const connection = {
      execute: vi.fn(() => Promise.resolve([{ affectedRows: 1 }])),
    };

    const affected = await deleteAttendance(
      connection as unknown as PoolConnection,
      "U1",
      "C1",
      "2024-01-01",
    ...
    expect(connection.execute).toHaveBeenCalledWith(
      expect.stringContaining("DELETE FROM bd_attendance"),
      ["C1", "2024-01-01", "U1"],
    );
  });
});
...
describe("main", () => {
  it("removes attendance when a backblast exists", async () => {
  ...
  it("fails when attendance is not present", async () => {
    process.env.MYSQL_URL = "mysql://user:pass@localhost/db";
    process.argv = ["node", "script.ts", "U02", "C02", "2024-01-01"];
